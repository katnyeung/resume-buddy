...
function SkillsTable({ skills, evidenceData, missingSkills }: {
  skills: any[],
  evidenceData?: any[],
  missingSkills?: any[]
}) {
  ...
  const getSuggestion = (evidenceStrength: string, isPrimary: boolean, linesShowcasing: number, tasksLinked: number): string => {
    ...
    return '✅ Strong evidence - multiple resume examples demonstrate this skill.';
  };
                    ...
                    {skill.taskNames && skill.taskNames.length > 0 && (
                      <div className="mt-2">
                        <ul className="list-disc list-inside space-y-1 text-indigo-700">
                          {skill.taskNames.slice(0, 3).map((task: string, i: number) => (
                            <li key={i}>{task}</li>
                          ))}
                          {skill.taskNames.length > 3 && (
                            <li className="italic">...and {skill.taskNames.length - 3} more</li>
                          )}
                        </ul>
                      </div>
                    )}
